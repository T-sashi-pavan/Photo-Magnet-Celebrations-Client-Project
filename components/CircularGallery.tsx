...
class App {
  ...
  onWheel(e: any) {
    this.autoRotate = false; // Stop auto-rotation when user scrolls
  ...
  update() {
    // Auto-rotate if enabled and user is not interacting
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (containerRef.current) {
        ...
        (containerRef.current as any)._app = app;
      }
    }, 100);
    
    return () => {
      clearTimeout(timer);
      if (containerRef.current && (containerRef.current as any)._app) {
        (containerRef.current as any)._app.destroy();
      }
    };
